 ...
 */
NS_LOG_COMPONENT_DEFINE ("dongkyunDL");
...
main (int argc, char *argv[])
{
  ...
  uint32_t udpPacketSizeURLLC = 128;
  uint32_t udpPacketSizeEMBB = 1280;
  uint32_t lambdaURLLC = 500000;
  uint32_t lambdaEMBB = 12500;
  ...
  UEURLLCNodes.Create(1);
  UEEMBBNodes.Create(1);
  enbNodes.Create(1);
  ...
  ueMobility.Install(UEURLLCNodes);
  ueMobility.Install(UEEMBBNodes);

  UEURLLCNodes.Get(0)->GetObject<MobilityModel> ()->SetPosition (Vector (-10, -1, 1.5));
  UEEMBBNodes.Get(0)->GetObject<MobilityModel> ()->SetPosition (Vector (-10, 1, 1.5));
  ...
  uint32_t bwpIdForURLLC = 0;

  // gNb routing between Bearer and bandwidh part
  nrHelper->SetGnbBwpManagerAlgorithmAttribute ("GBR_CONV_VOICE", UintegerValue (bwpIdForEMBB));
  nrHelper->SetGnbBwpManagerAlgorithmAttribute ("GBR_CONV_VOICE", UintegerValue (bwpIdForURLLC));

  // Ue routing between Bearer and bandwidth part
  nrHelper->SetUeBwpManagerAlgorithmAttribute ("GBR_CONV_VOICE", UintegerValue (bwpIdForEMBB));
  nrHelper->SetUeBwpManagerAlgorithmAttribute ("GBR_CONV_VOICE", UintegerValue (bwpIdForURLLC));
  ...
  NetDeviceContainer enbNetDev = nrHelper->InstallGnbDevice (enbNodes.Get(0), allBwps);
  NetDeviceContainer ueURLLCNetDev = nrHelper->InstallUeDevice (UEURLLCNodes, allBwps);
  ...
  for (auto it = ueURLLCNetDev.Begin (); it != ueURLLCNetDev.End (); ++it)
    {
      DynamicCast<NrUeNetDevice> (*it)->UpdateConfig ();
    }

  for (auto it = ueEMBBNetDev.Begin (); it != ueEMBBNetDev.End (); ++it)
    {
      DynamicCast<NrUeNetDevice> (*it)->UpdateConfig ();
    }
  ...
  internet.Install (UEURLLCNodes);
  internet.Install (UEEMBBNodes);


  Ipv4InterfaceContainer ueURLLCIpIface = epcHelper->AssignUeIpv4Address (NetDeviceContainer (ueURLLCNetDev));
  Ipv4InterfaceContainer ueEMBBIpIface = epcHelper->AssignUeIpv4Address (NetDeviceContainer (ueEMBBNetDev));

  // Set the default gateway for the UEs
    Ptr<Ipv4StaticRouting> ueStaticRouting1 = ipv4RoutingHelper.GetStaticRouting (UEURLLCNodes.Get (0)->GetObject<Ipv4> ());
    ueStaticRouting1->SetDefaultRoute (epcHelper->GetUeDefaultGatewayAddress (), 1);
    
    Ptr<Ipv4StaticRouting> ueStaticRouting2 = ipv4RoutingHelper.GetStaticRouting (UEEMBBNodes.Get (0)->GetObject<Ipv4> ());
    ueStaticRouting2->SetDefaultRoute (epcHelper->GetUeDefaultGatewayAddress (), 1);
  // attach UEs to the closest eNB
  nrHelper->AttachToClosestEnb (ueURLLCNetDev, enbNetDev);
  nrHelper->AttachToClosestEnb (ueEMBBNetDev, enbNetDev);
  /*
  ...
  serverApps.Add (dlPacketSinkURLLC.Install (UEURLLCNodes));
  serverApps.Add (dlPacketSinkEMBB.Install (UEEMBBNodes));
  ...
  dlClientURLLC.SetAttribute ("PacketSize", UintegerValue (udpPacketSizeURLLC));
  dlClientURLLC.SetAttribute ("Interval", TimeValue (Seconds (1.0 / lambdaURLLC)));
  ...
  dlClientEMBB.SetAttribute ("PacketSize", UintegerValue (udpPacketSizeEMBB));
  dlClientEMBB.SetAttribute ("Interval", TimeValue (Seconds (1.0 / lambdaEMBB)));
  ...
  Ptr<Node> ue1 = UEURLLCNodes.Get (0);
  Ptr<NetDevice> ueDevice1 = ueURLLCNetDev.Get (0);
  Address ueAddress_URLLC = ueURLLCIpIface.GetAddress (0);

  // The client, who is transmitting, is installed in the remote host,
  // with destination address set to the address of the UE
  dlClientURLLC.SetAttribute ("RemoteAddress", AddressValue (ueAddress_URLLC));
  clientApps.Add (dlClientURLLC.Install (remoteHost));

  nrHelper->ActivateDedicatedEpsBearer (ueDevice1, URLLCBearer, URLLCTft); 

  Ptr<Node> ue2 = UEEMBBNodes.Get (0);
  Ptr<NetDevice> ueDevice2 = ueEMBBNetDev.Get (0);
  Address ueAddress_EMBB = ueEMBBIpIface.GetAddress (0);

  // The client, who is transmitting, is installed in the remote host,
  // with destination address set to the address of the UE
  dlClientEMBB.SetAttribute ("RemoteAddress", AddressValue (ueAddress_EMBB));
  clientApps.Add (dlClientEMBB.Install (remoteHost));

  nrHelper->ActivateDedicatedEpsBearer (ueDevice2, EMBBBearer, EMBBTft);
  ...
  endpointNodes.Add (UEURLLCNodes.Get(0));
  endpointNodes.Add (UEEMBBNodes.Get(0));
